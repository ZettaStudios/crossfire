package to.zettagames.crossfire.emulator.shared.util;
...
public class Binary {
    ...
    public static float readFloat(byte[] bytes, int accuracy) {
        ...
        return accuracy > -1 ? (float) Math.round(val, accuracy) : val;
    }
    ...
    public static byte[][] splitBytes(byte[] bytes, int chunkSize) {
        ...
        for (int i = 0; i < bytes.length; i += chunkSize) {
            splits[chunks] = Arrays.copyOfRange(bytes, i, (bytes.length - i) > chunkSize ? i + chunkSize : bytes.length);
            chunks++;
        }
    ...
    public static byte[] StringToBytes(String str, boolean endsWithF2) {
        byte[] result = str.getBytes(StandardCharsets.US_ASCII);
        if (endsWithF2) result[result.length - 1] = (byte) 0xF2;
        return result;
    }
    ...
    public static String BytesToString(byte[] buffer) {
        ...
        return builder.delete(builder.length() - 1, 1).toString();
    }
